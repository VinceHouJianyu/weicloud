  ...
  <el-container  >
    ...
    <el-main>
      ...
      <div class="text item">
        ...
        商品说明
          优势：
            节能降耗：根据业务模型精细化控制高能耗设备
            工艺参数优化：基于制造过程、环境、售后数据，分析问题发生的环节和工艺参数优化点
            预测性维护：根据设备过去和现在的状态，预测系统将来是否会发生故障，何时发生故障
            销售预测：基于销售、节假日、天气数据，预测产品销量，降低备货和库存成本
            产品质量优化提升：基于客户的反馈、互联网点评分析、竞争对手分析、维修记录、售后历史数据，进行分类分析，发现产品关键问题，指导新产品设计改善，提升产品质量
            智能设备维护：预测性维护，根据系统过去和现在的状态，采用时间序列预测、神经网络预测和回归分析等预测推理方法，预测系统将来是否会发生故障，何时发生故障，发生故障类型，可以提升服务运维效率，降低设备非计划停机时间，节约现场服务人力成本
            生产物料预估：基于历史物料数据，对生产所需物料进行准确分析预估，降低仓储周期，提升效率
...
<script>
  ...
  export default {
    mounted: function () {
      var _this = this;
      getRequest("/product/getOneProduct?product_id=1").then(resp => {
        if (resp.status == 200) {
