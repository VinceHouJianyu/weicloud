    ...
    <el-main>
      ...
      <div class="text item">
        <pre>
          ...
          工业IOT平台解决方案	——提供数据存储、数据处理和应用接口对接等
          工业IOT解决方案	——为装备制造业提供专业智能服务
          工业互联网平台	——针对工业制造环节，提供大数据服务
          机加工协同智造解决方案	——提供设备生产监控和协同制造的平台
...
<script>
  ...
  export default {
    mounted: function () {
      var _this = this;
      getRequest("/product/getOneProduct?product_id=3").then(resp => {
        if (resp.status == 200) {
