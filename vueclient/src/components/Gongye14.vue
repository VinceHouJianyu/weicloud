    ...
    <el-main>
      ...
      <div class="text item">
        <pre>
          ...
          应用迁移服务	1．B/S架构Web网站应用和数据库上云迁移服务
                      2．专业的项目管理服务与完整的应急措施
                      3．提供专业工具、专家支撑、质量管理
          数据库迁移服务	1．本地或云上MySQL等数据库迁移到云上数据库
                      2．提供环境配置、网络配置和安全配置服务
                      3．一次性投入，通过规模化迁移降低成本
          专家迁移服务	1．为客户综合复杂系统迁移提供一对一专家服务
                      2．提供业务设计、TCO评估报告、规划上云策略
                      3．根据客户需求，可提供现场服务
...
<script>
  ...
  export default {
    mounted: function () {
      var _this = this;
      getRequest("/product/getOneProduct?product_id=30").then(resp => {
        if (resp.status == 200) {
