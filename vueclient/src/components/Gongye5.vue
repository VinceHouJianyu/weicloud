    ...
    <el-main>
      ...
      <div class="text item">
        <pre>
          ...
          在线CAD解决方案	——提供一站式在线CAD设计平台
          仿真云解决方案	——构建高性能仿真云计算环境
          全生命周期管理系统	——致力于从多方面改善企业研发体系
          PLM 协同管理软件	——部门间协作及产品数据全局管理共享
...
<script>
  ...
  export default {
    mounted: function () {
      var _this = this;
      getRequest("/product/getOneProduct?product_id=18").then(resp => {
        if (resp.status == 200) {
